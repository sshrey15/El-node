...
import { Plus, Search, Package, Upload, Trash2, Edit, Loader2, X, ImageIcon } from "lucide-react"
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
                ...
                <TableBody>
                  {filteredProducts.map((product) => (
                    <TableRow key={product.id}>
                      <TableCell>
                        <div className="w-12 h-12 flex items-center justify-center bg-muted rounded-lg overflow-hidden relative">
                          {product.image ? (
                            <Image
                              src={product.image}
                              alt={product.name}
...
function ImageUploadComponent({ 
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // More aggressive compression to avoid 413 errors
      const options = {
        maxSizeMB: 0.2, // Reduced to 200KB
      ...
      if (compressedFile.size > 500000) { // 500KB limit
      ...
      alert(error instanceof Error ? error.message : "Error processing image. Please try a smaller image.")
    } finally {
  ...
  return (
    <div className="space-y-2">
        ...
        {!disabled && (
          <div className="flex items-center space-x-2">
            ...
            >
              ...
              {isUploading ? "Processing..." : currentImageSrc ? "Change Image" : "Upload Image"}
            </label>
            <input
              id="image-upload"
              type="file"
      ...
      <p className="text-xs text-muted-foreground">
        Upload an image for this product (JPG, PNG, GIF - will be compressed to under 200KB)
      </p>
    </div>
  )
}
